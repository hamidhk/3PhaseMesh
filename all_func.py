...
def verticesLaplaceBeltramiNeighborhood0(faces, numVerts):
    # for every vertex, finds all faces at the 1-ring
    # neighborhood of vertex. Then, using this,
    # returns also vertices neighbor with vertex;
    # returns also a np.array with 4 axes, where
...
def meshA_meshB_common_surface0(vertsW, facesW, vertsN, facesN):
...
def meshA_meshB_common_surface1(vertsA, facesA, vertsB, facesB):
...
def meshA_meshB_common_surface4(vertsA, facesA, vertsB, facesB):
    # receives verts/faces of mesh A & mesh B for two neighbor volumes
    # returns mesh (verts/faces) for  AB common surface
...
def meshA_meshB_commonSurface_5(vertsA, facesA, vertsB, facesB):
    # receives verts/faces of mesh A & mesh B for two neighbor volumes
    # returns mesh (verts/faces) for  AB common surface
...
def smoothingWeightsAndCurvatures0(verts, nV, nbrLB, *args):
...
def MeanGaussianPrincipalCurvatures1(verts, nV, nbrLB, *args):
    # returns a weight func.(wf) for smoothing triangle mesh data
...
def MeanGaussianPrincipalCurvatures2(verts, nV, nbrLB, *args):
    # returns a weight func.(wf) for smoothing triangle mesh data
    # returns also max triangle area
    # defualt wf is the mean curvature as in isotropic diffusion smoothing.
    # wf is calculated by anisotropic diffusion method, only 
...
def meanGaussianPrincipalCurvatures3(verts, nV, nbrLB, **kwargs):
    # returns a weight func.(wf) for smoothing triangle mesh data
    # returns also max triangle area
    # defualt wf is the mean curvature as in isotropic diffusion smoothing.
    # wf is calculated by anisotropic diffusion method, only 
...
def MeanCurvatures0(verts, nV, nbrLB):
...
def MeanCurvatures1(verts, nV, nbrLB):
    # this func. is copied from MeanGaussianPrincipalCurvatures()
    # returns mean curvature only
    # For details of the method, check:
    # "Discrete Differential-Geometry Operators for Triangulated 2-Manifolds"
...
def smoothing0(verts, faces, nbrLB, *args):
...
def smoothing1(verts, faces, nbrLB, **kwargs):
...
def smoothing2(verts, faces, nbrLB, **kwargs):
...
def smoothing3(verts, faces, nbrLB, **kwargs):
    # smooths a triangulated mesh e.g.the WN-interface
    # receives verts, faces, & Laplace-Beltrami neighborhood map (nbrLB) of verts
    # returns smoothed verts
    print('\nsmoothing iteration num.')
...
def smoothingStDev0(verts, faces, nbrLB, **kwargs):
    # smoothes a triangulated mesh e.g.the WN-interface
    # receives verts, faces, & Laplace-Beltrami neighborhood map (nbrLB) of verts
    # returns smoothed verts
    print('\nsmoothing iteration num.')
...
def smoothingStDev1(verts, faces, nbrLB, **kwargs):
    # smooths a triangulated mesh e.g.the WN-interface
    # receives verts, faces, & Laplace-Beltrami neighborhood map (nbrLB) of verts
    # returns smoothed verts
    print('\nsmoothing iteration num.')
    ...
    while condition:    # smoothing loop
        ...
        verts_itr = VV[mm-1] - 0.1*tune
        # comparing verts_itr with the originals & correcting the jumped ones
        # constraint is to avoid displacement more than DD at every vert
        # if disp. is more than DD, vertex goes back to value @ previous iter.
        dd = sum(((verts_original - verts_itr)**2).T) # squared of distances
        jump = dd >= DD**2
        verts_itr[jump] = VV[mm-1][jump]
...
def smoothingStDev2(verts, faces, nbrLB, **kwargs):
    # smooths a triangulated mesh e.g.the WN-interface
    # receives verts, faces, & Laplace-Beltrami neighborhood map (nbrLB) of verts
    # returns smoothed verts
    print('\nsmoothing iteration num.')
